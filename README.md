...
model transfer. DAXIS answers, in one pass over your features, the question
...
==================================================================
  ...
  mode      : binary
  DAXIS score: +0.78   (95% CI [+0.71, +0.84])
...
```bash
pip install daxis                 # core: numpy, scipy, scikit-learn
pip install daxis[plot]           # + matplotlib for .plot()
pip install daxis[torch]          # + the PyTorch adapter
pip install daxis[tf]             # + the TensorFlow adapter
```
...
For each domain `b`, DAXIS forms the **unit class-discriminant direction**
...
*DAXIS score*. Cosine ≈ **+1**: every domain separates the classes along the
...
DAXIS auto-selects, but you can force it:
...
```python
daxis.daxis_score(X, y, domain, mode="auto")        # binary if 2 classes, else classwise
daxis.daxis_score(X, y, domain, mode="subspace")    # multiclass, few domains
daxis.daxis_score(X, y, domain, mode="classwise")   # deep features
```
...
Freeze a backbone, let DAXIS pull the features out of the loader you already have:

```python
import torch, torchvision as tv, daxis
...
```python
import daxis
res = daxis.from_tensorflow(ds, backbone=feature_model,
...
```python
import pandas as pd, daxis
res = daxis.from_dataframe(df, label_col="y", domain_col="site")
```

### Command line

```bash
daxis features.csv --label y --domain site --plot matrix.png --json out.json
daxis features.npz --mode classwise          # npz with arrays X, y, domain
```
...
The verdict is geometry, not a promise of accuracy: DAXIS isolates *whether a
shared boundary exists to be aligned*. It is complementary to distance-based
transferability scores (which measure how far the domains are) — DAXIS measures
whether moving them closer can possibly help.

## API

```
daxis.daxis_score(X, y, domain, mode="auto", n_boot=500, n_perm=500,
                standardize=True, random_state=0, ci=95.0) -> DAXISResult
daxis.from_torch(loader, backbone=None, device="cpu", domain_fn=None, ...)
daxis.from_tensorflow(ds, backbone=None, domain_fn=None, ...)
daxis.from_dataframe(df, feature_cols=None, label_col="label", domain_col="domain", ...)

DAXISResult.score / .ci / .p_value / .regime / .matrix / .domains
DAXISResult.report()     -> str
DAXISResult.plot(path)   -> saves the cosine heatmap
DAXISResult.to_dict()    -> JSON-serialisable
```

## Citation

```bibtex
@article{dasilva2026daxis,
  title   = {DAXIS: A Discriminant-Axis Alignment Diagnostic for Deciding
